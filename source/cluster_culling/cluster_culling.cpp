...
LRESULT ClusterCulling::MsgProc(HWND hwnd, uint msg, WPARAM wParam, LPARAM lParam) {
...
ClusterCulling::ClusterCulling(HINSTANCE hinstance)
	: Engine::HalflingEngine(hinstance),
	  ...
	  m_lightCullingPlanesNeedUpdate(true),
	  m_vsync(false),
	  m_wireframe(false),
...
void ClusterCulling::OnResize() {
	...
	// Resize the culling planes
	uint numClusters_X = (m_clientWidth + COMPUTE_SHADER_TILE_GROUP_DIM - 1) / COMPUTE_SHADER_TILE_GROUP_DIM;
	uint numClusters_Y = (m_clientHeight + COMPUTE_SHADER_TILE_GROUP_DIM - 1) / COMPUTE_SHADER_TILE_GROUP_DIM;
	uint numClusters_Z = 64u;

	m_lightCullingPlanes_X.resize(numClusters_X + 1);
	m_lightCullingPlanes_Y.resize(numClusters_Y + 1);
	m_lightCullingPlanes_Z.resize(numClusters_Z + 1);
...
void ClusterCulling::MouseDown(WPARAM buttonState, int x, int y) {
	m_mouseLastPos.x = x;
	m_mouseLastPos.y = y;
...
void ClusterCulling::MouseUp(WPARAM buttonState, int x, int y) {
	ReleaseCapture();
}

void ClusterCulling::MouseMove(WPARAM buttonState, int x, int y) {
