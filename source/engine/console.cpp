...
void Console::Initialize(Common::Rect rect, Graphics::SpriteRenderer *spriteRenderer, Graphics::SpriteFont *font, uint maxNumLines) {
	m_rect = rect;
...
void Console::Render(ID3D11DeviceContext *context, double deltaTime) {
	...
	m_spriteRenderer->Begin(context, Graphics::SpriteRenderer::Point);
...
void Console::PrintText(const std::wstring &line) {
	...
	for (uint i = 0; i < line.size(); ++i) {
		...
		default:
		{
			Graphics::SpriteFont::CharDesc desc = m_font->GetCharDescriptor(character);
