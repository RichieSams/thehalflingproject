...
void SpriteRenderer::Initialize(ID3D11Device *device) {
	...
	// Load the shaders
	HR(LoadVertexShader(L"sprite_vertex_shader.cso", device, &m_vertexShader, &m_inputLayout, layout, 2));
	HR(LoadVertexShader(L"sprite_instanced_vertex_shader.cso", device, &m_vertexShaderInstanced, &m_inputLayoutInstanced, layoutInstanced, 8));
	HR(LoadPixelShader(L"sprite_pixel_shader.cso", device, &m_pixelShader));
	HR(LoadPixelShader(L"sprite_sampled_pixel_shader.cso", device, &m_sampledPixelShader));
